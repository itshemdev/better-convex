...
- `polymorphic`: synthesize a discriminated-union relation field from `one()` relations
...
await db.query.comments.findMany({
  polymorphic: {
    ...
    // as: 'target', // default alias is "target"
  },
  limit: 20,
});
```
...
- `discriminator` must match the schema discriminator key
- `cases` keys must match discriminator literals
- `cases` values must be `one()` relation names on the source table
- Runtime mismatch/parse failures throw immediately
- Auto-loaded case relations are stripped unless explicitly requested in `with`
- `pipeline` cannot be combined with `polymorphic`

`polymorphic` is also available on `findFirst()` and `findFirstOrThrow()`.
